...
let rec lookup (k: int) (l: expr) : expr = 
  match l with
  | Nil(TyPair(TyInt, TyInt)) -> Nothing(TyInt) (* Retorna Nothing se a lista for vazia *)
  | Cons(Pair(Num(k1), Num(v1)), xs) -> 
      if k = k1 then 
        Just(Num(v1))  (* Retorna Just com o valor correspondente ao par encontrado *)
      else 
        lookup k xs  (* Continua buscando na lista *)
...
let result = match lookup 3 base_dados with
  | Nothing _ -> Num(0)
  | Just(Num n) -> Num(n)
...
(* ------------------------------------------------------------------------------- *) 
let rec tyList_max (l: expr) : expr =
  match l with
  | Nil(TyInt) -> Nothing(TyInt)
  | Cons(Num(h), t) -> (
      match tyList_max t with
      | Nothing _ -> Just(Num(h))
      | Just(Num(m)) -> Just(Num(if h >= m then h else m))
      | _ -> failwith "Unexpected match case"
    )
  | _ -> failwith "Tipo inválido"
